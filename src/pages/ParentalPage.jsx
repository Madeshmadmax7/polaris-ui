...
export default function ParentalPage() {
    ...
    async function handleCancelPending(notificationId, connectionId) {
        ...
        try {
            ...
            setPendingConnections(pending => 
                pending.filter(n => n.id !== notificationId)
            );
        } catch (err) {
    ...
    function handleRequestSent(payload) {
        ...
        // Add immediately so user sees Enter OTP without manual refresh.
        setPendingConnections((prev) => [localPending, ...prev]);
        setShowConnectModal(false);

        // Sync with backend authoritative pending list.
        setTimeout(() => {
            loadPendingConnections();
        }, 400);
    }
    ...
    async function handleVerifyOtpClick() {
        // Opens modal for parent to enter OTP they received from child
        setConnectionData({
            connection_id: null,
            expires_in_seconds: 120
        });
    ...
    async function handleVerifyConnection(result) {
        if (result.success) {
            alert('Connection verified! You can now view child analytics.');
            await loadChildren();
    ...
    async function handleDisconnect(connectionId) {
        if (window.confirm('Disconnect from this child?')) {
            try {
    ...
    return (
        <div className="container mx-auto px-6 py-16 max-w-7xl animate-in font-outfit">
            ...
            {/* OTP Connections Section */}
            <div className="mb-16">
                <div className="flex items-center gap-4 mb-8">
                    <Link2 size={20} className="text-white" />
                    <div>
                        <h2 className="text-2xl font-semibold tracking-tight text-white">Verified Connections</h2>
                        <p className="text-zinc-500 text-[12px] mt-1">OTP-based verified connections for analytics access | Valid for 30 days</p>
                    </div>
                </div>

                {/* Active Connections - Horizontal Grid */}
                {connections.length > 0 && (
                    <div className="mb-8">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                            {connections.map(conn => (
                                <div key={conn.connection_id} className="p-6 bg-gradient-to-br from-green-950/30 to-green-950/10 border border-green-900/30 rounded-[32px] hover:border-green-900/60 transition-all group">
                ...
                <button
                    onClick={() => {
                        setConnectionData({ connection_id: null, child_email: '', expires_in_seconds: 120 });
                        setShowOtpModal(true);
                    }}
                    className="w-full md:w-auto mt-3 md:mt-0 md:ml-3 px-8 py-4 bg-black hover:bg-zinc-900 border border-white text-white font-bold uppercase tracking-[0.3em] text-[10px] rounded-[32px] flex items-center justify-center gap-3 transition-all"
                >
                    <Key size={16} />
                    I Have OTP
                </button>
                ...
                {pendingConnections.length > 0 && (
                    <div className="mt-12 pt-12 border-t border-white/10">
                        <div className="mb-6">
                            <h3 className="text-lg font-semibold text-white mb-2">Waiting for OTP</h3>
                            <p className="text-zinc-500 text-[12px]">Enter the 4-digit OTP your child shared with you</p>
                        </div>
                        <div className="space-y-3">
                            {pendingConnections.map(notif => (
                                <div key={notif.id} className="p-4 bg-yellow-950/20 border border-yellow-900/30 rounded-[24px] flex items-center justify-between">
                                    <div>
                                        <p className="text-sm font-medium text-white">{notif.data?.child_name || 'Child'}</p>
                                        <p className="text-[11px] text-zinc-400 mt-1">Waiting for OTP verification</p>
                                    </div>
