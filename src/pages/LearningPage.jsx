...
export default function LearningPage() {
    ...
    // New feature state
    const [summaries, setSummaries] = useState({});         // chapter_number → ai_summary text
    const [quizCooldown, setQuizCooldown] = useState(null); // {hours, minutes, attempts_used, max_retakes}
    const [regenerating, setRegenerating] = useState(false);
    // Post-quiz agent analysis modal states — all recommendations are manual-approval only
    const [postQuizModal, setPostQuizModal] = useState(null);     // { score, correct, total }
    const [agentAnalysis, setAgentAnalysis] = useState(null);     // 'loading' | analysis-object | null
    const [showContinueForm, setShowContinueForm] = useState(false);
    const [continueParams, setContinueParams] = useState({ days: 14, difficulty: 'medium' });
    ...
    const handleSubmitQuiz = async () => {
        ...
        try {
            ...
            // Trigger agent analysis in background — modal shows loading state immediately
            const currentPlan = studyPlans.find(p => p.id === selectedPlan);
            setPostQuizModal({ score: result.score, correct: result.correct_answers, total: result.total_questions });
            setAgentAnalysis('loading');
            setShowContinueForm(false);
            ai.analyzeQuizResult(selectedPlan).then(analysis => {
                setAgentAnalysis(analysis);
                setContinueParams({
                    days: analysis.recommended_total_days || currentPlan?.duration_days || 14,
                    difficulty: analysis.recommended_difficulty || 'medium',
                });
            }).catch(() => {
                setAgentAnalysis({
                    weak_topics: [],
                    reasoning: 'Could not load AI analysis. Review your wrong answers manually.',
                    agent_message: 'Focus on the topics you found difficult.',
                    should_continue: result.score < 75,
                    recommended_total_days: currentPlan?.duration_days || 14,
            ...
            if (err.status === 429 && err.rawDetail && typeof err.rawDetail === 'object') {
                setQuizCooldown({
                    hours: err.rawDetail.cooldown_remaining_hours ?? 0,
                    minutes: err.rawDetail.cooldown_remaining_minutes ?? 0,
                    attempts_used: err.rawDetail.attempts_used ?? 3,
                    max_retakes: err.rawDetail.max_retakes ?? 3,
                });
            } else {
                alert(err.message);
            }
    ...
    const handleRegeneratePlan = async () => {
        if (!selectedPlan) return;
        if (!confirm('Regenerate this plan? The AI will create a revised version using your learning history — skipping topics you already mastered. All chapter progress will be reset.')) return;
        setRegenerating(true);
        try {
            const updatedPlan = await ai.regenerateStudyPlan(selectedPlan);
            setStudyPlans(studyPlans.map(p => p.id === selectedPlan ? updatedPlan : p));
            setPlanProgress(null);
            setShowQuiz(false);
            setQuizResult(null);
            await loadPlanProgress(selectedPlan);
            alert('✓ Plan regenerated! The revised version reflects your learning progress.');
        } catch (err) {
            alert(err.message);
        } finally {
    ...
    const handleConfirmRegenerate = async () => {
        ...
        try {
            const updatedPlan = await ai.regenerateStudyPlan(selectedPlan, {
                override_duration_days: continueParams.days,
                override_difficulty: continueParams.difficulty,
                weak_topics: agentAnalysis?.weak_topics || [],
            });
            setStudyPlans(studyPlans.map(p => p.id === selectedPlan ? updatedPlan : p));
            setPlanProgress(null);
            setShowQuiz(false);
    ...
    return (
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10 mb-12">
                            ...
                            <div className="flex flex-col items-end gap-4">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/10 text-[10px] font-bold uppercase tracking-widest text-zinc-400 hover:text-white hover:border-white/30 transition-all disabled:opacity-40"
                                    title="Regenerate this plan — AI will create a revised version using your learning history"
                                >
                                    ...
                                    {chapters.map((chapter) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {/* AI Summary Panel — auto-generated after chapter completion */}
                                                {isCompleted && (
                                                    <div className="mt-4 border-t border-white/5 pt-4">
                                                        <button
                                                            onClick={async () => {
                                                                const n = chapter.chapter_number;
                                                                if (summaries[n] !== undefined) {
                                                                    setSummaries(s => ({ ...s, [n]: summaries[n] === null ? undefined : null }));
                                                                    return;
                                                                }
                                                                setSummaries(s => ({ ...s, [n]: 'loading' }));
                                                                try {
                                                                    const data = await ai.getChapterSummary(selectedPlan, n);
                                                                    setSummaries(s => ({ ...s, [n]: data.ai_summary || 'No summary yet. Complete and re-open to auto-generate.' }));
                                                                } catch {
                                                                    setSummaries(s => ({ ...s, [n]: 'Could not load summary.' }));
                                                                }
                                                            }}
                                                        ...
                                                        >
                                                            <span>📋</span>
                                                            {summaries[chapter.chapter_number] === null ? 'Hide Summary' : 'What I Learned'}
                                                        </button>
                                                        {summaries[chapter.chapter_number] && summaries[chapter.chapter_number] !== null && (
                                                            <div className="mt-3 p-4 bg-black/40 border border-white/5 rounded-2xl">
                                                                {summaries[chapter.chapter_number] === 'loading' ? (
                                                                    <span className="text-zinc-700 text-xs">Generating summary…</span>
                                                                ) : (
                                                                    <pre className="text-zinc-400 text-[11px] font-light whitespace-pre-wrap leading-relaxed font-sans">{summaries[chapter.chapter_number]}</pre>
                                                                )}
                                                            </div>
                                                        )}
                                                    </div>
        ...
        {/* ============================================================
            POST-QUIZ AGENT ANALYSIS MODAL
            Showed automatically after every quiz submission.
            AI recommendations are displayed for review — nothing is
            applied until the user explicitly clicks “Confirm & Regenerate”.
        ============================================================ */}
        {postQuizModal && (
