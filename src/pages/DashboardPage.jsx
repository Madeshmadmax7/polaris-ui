...
import { productivity, parental, tracking, connectDashboardWS } from '../api';
...
export default function DashboardPage({ childId = null, childName = null, parentView = false }) {
    const { user } = useAuth();
    ...
    const isChildProxy = parentView && !!childId;
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                const [t, tr, stats] = await Promise.all(
                    isChildProxy
                        ? [
                            parental.getChildToday(childId).catch(() => null),
                            parental.getChildTrend(childId, 14).catch(() => null),
                            parental.getChildDashboardStats(childId, { start: todayStr }).catch(() => null),
                        ]
                        : [
        ...
        load();
    }, [isChildProxy, childId]);

    // WebSocket for live tracking updates
    useEffect(() => {
        if (isChildProxy) return;
                ...
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 px-1 font-outfit">
                    <div>
                        ...
                        <h1 className="text-3xl font-light tracking-tight text-white">
                            Welcome, <span className="font-semibold">{isChildProxy ? (childName || 'Child') : user?.username}</span>
                        </h1>
                    </div>
        ...
        <>
            ...
            >
                ...
                <td className="px-4 py-6 text-center">
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-wider border ${site.category === 'productive' ? 'bg-white text-black border-white' :
